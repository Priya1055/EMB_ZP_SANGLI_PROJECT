...
if( $embsection2->mb_status >= 8)
{
                ...
                $SDCFHCODEId=$DBSDCgetdata->F_H_Code;
                ...
                $SDCFHCODE=DB::table('fundhdms')->where('F_H_CODE',$SDCFHCODEId)->value('Fund_Hd_M');
...
$subdivisioncheckreport .= '<div style="position: absolute; top: 12%; left: 90%; transform: translateX(-50%); font:weight;">' . $qrCodeString . '</div><div class="" style="margin-top:20px; border-collapse: collapse;">';
  ...
  <td  style="">' . $workdata->Agency_Nm . '</td>
   ...
   $subdivisioncheckreport .= '<tr>';
   $subdivisioncheckreport .= '<td colspan="2" style="width: 60%;"><strong>WorkOrder No:</strong>' . $workdata->WO_No . '</td>';
   $subdivisioncheckreport .= '<td colspan="" style="width: 40%; text-align: right;"><strong>WorkOrder Date:</strong>' . $workorderdt . '</td>';
   $subdivisioncheckreport .= '</tr>';
...
// // Set options for the PDF rendering
// $options = new Options();
...
// // Set font for rendering Marathi characters
// $font = $pdf->getFontMetrics()->getFont('Arial');
    ...
    if( $embsection2->mb_status >=11 )
     {
        ...
        $work_id = $BillsData->work_id;
        $workNM=$DBAudiExist->Work_Nm;
        $SDCFHCODENO= $DBAudiExist->F_H_Code;
        $FH_code=DB::table('fundhdms')->where('F_H_CODE',$SDCFHCODENO)->value('Fund_Hd_M');
        $Arith_chk = $DBAudiExist->Arith_chk;
        $Ins_Policy_Agency = $DBAudiExist->Ins_Policy_Agency ;
        $Ins_Prem_Amt_Agency = $DBAudiExist->Ins_Prem_Amt_Agency;
        $Bl_Rec_Ded = $DBAudiExist->Bl_Rec_Ded ;
...
$DivisionCheck .= '<div style="position: absolute; top: 12%; left: 90%; transform: translateX(-50%); font:weight;">' . $qrCodeString . '</div><div class="" style="margin-top:20px; border-collapse: collapse;">';
  ...
  <td  style="">' . $workdata->Agency_Nm . '</td>
   ...
   $DivisionCheck .= '<tr>';
   $DivisionCheck .= '<td colspan="2" style="width: 60%;"><strong>WorkOrder No:</strong>' . $workdata->WO_No . '</td>';
   $DivisionCheck .= '<td colspan="" style="width: 40%; text-align: right;"><strong>WorkOrder Date:</strong>' . $workorderdt . '</td>';
   $DivisionCheck .= '</tr>';
