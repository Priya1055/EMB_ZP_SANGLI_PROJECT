...
//dd($workid);
        ...
        // if ($billitems > 0) {
            ...
            {
            //dd($billitems);
            // Session::flash('error', 'Cannot delete the bill. Bill items exist.');
      ...
      //  dd($lastbilldata->Bill_Dt , $lastbilldt , $workorderdt);
       ...
       //dd($mindt);
         ...
         $minimumdate = date('Y-m-d', strtotime($minimumdt));
            ...
            //dd($lasBill);
      //dd($maxDate  ,$minDate,$minimumdt);
     // dd($maximumdt , $minimumdate,  $lastbilldt , $date180DaysAgo);
...
// Query using the retrieved workId
$employee = DB::table('workmasters')->where('Work_Id', $workId)->get();
//dd($employee);
            return view('trial', compact('employee'));
          }
          // Generate PDF
     ...
     //view document of bill for EE and DE
     ...
     //image documents video delete function 
    ...
    // function for the work haNDOVER CERTIFICATE 
        ...
        if ($request->File)
        {
            ...
            $file->move(public_path('Uploads/Workhandovercertificates'), $pakageimage1Name);
            ...
            $updatedocuments = DB::table('bills')->where('t_bill_id', $tbillid)->update([
                'WHOCdocument' => $pakageimage1Name,
            ]);
           }
