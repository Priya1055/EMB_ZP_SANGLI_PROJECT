...
return $importedData;
  ...
  public function readersheet2($file,$dataWorkid)
    {
        // dd($dataWorkid);
        $excel = IOFactory::load($file);
...
//  dd($MAgree_Dt);
 ...
 if (!empty($MAgree_Dt)) {
    ...
    if (is_numeric($MAgree_Dt)) { 
        //  dd($MAgree_Dt);

        // Replace with your actual Excel date serial number
        $excelBaseDate = new DateTime('1900-01-01');
        // Calculate the number of days from the Excel base date
        $daysFromBaseDate = $MAgree_Dt - 1; // Excel is 1-based, so subtract 1
        // Create a new DateTime object using the calculated date
        $dateTimeObject = clone $excelBaseDate;
        $dateTimeObject->modify("+$daysFromBaseDate days");
        // Subtract one day to account for the Excel quirk
        $dateTimeObject->modify("-1 day");
        // Format the date as needed
        $MAgree_DtdateFormatted = $dateTimeObject->format('Y-m-d');
        // dd($MAgree_DtdateFormatted);
    } 
    else {
        // Attempt to create a DateTime object using the format 'd/m/Y'
        $dateTimeObject = DateTime::createFromFormat('d/m/Y', $MAgree_Dt);
        // Check if the conversion was successful
        if ($dateTimeObject !== false) {
            // Format the date as 'Y-m-d'
            $MAgree_DtdateFormatted = $dateTimeObject->format('Y-m-d');
...
//Request Workid Parameter is present
// if(isset( $dataWorkid) && !empty( $dataWorkid) &&  $dataWorkid !== null){
//     dd($dataWorkid);
//     $DBWork_Id = $dataWorkid;
// }
// else{
    // Old WorkId Genrate Code---------------------------------------------------------------------------
    if ($maxWorkId !== null) 
    {
...
elseif(isset($dataWorkid) && !empty($dataWorkid)) {
...
'WO_Amt'=> $MWO_Amount,
'Agree_No'=>$MAgree_no,
'Agree_Dt'=>$MAgree_DtdateFormatted,
'Tal_Id' => $DBTal_Id, 
'Tal'=> $MTaluka,
'Ps_Consti'=>$DBpsId,
'Zp_Consti'=>$DBzpId,
