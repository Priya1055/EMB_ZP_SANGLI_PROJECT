...
<div class="col-12 col-sm-4 col-md-4 col-lg-4">
    <select id="workType" class="form-control" style="max-width: 280px;" name="workType" >
        <option value="Road" {{ $DSWorkmaster->Work_Type == 'Road' ? 'selected' : '' }}>Road</option>
    <option value="Building" {{ $DSWorkmaster->Work_Type == 'Building' ? 'selected' : '' }}>Building</option>
    <option value="Bridge" {{ $DSWorkmaster->Work_Type == 'Bridge' ? 'selected' : '' }}>Bridge</option>
    <option value="CD" {{ $DSWorkmaster->Work_Type == 'CD' ? 'selected' : '' }}>CD</option>
    </select>
    ...
    <div class="col-12 col-md-3">
        <div class="form-group">
            ...
            <!-- <input type="text" id="AA_Date" name="AA_Date" class="form-control"
             ...
             <input type="date" id="AA_Date" name="AA_Date" class="form-control" placeholder="Enter A.A. Date"
       value="{{ ($DSWorkmaster->AA_Dt) }}" Max="{{$DSWorkmaster->TS_Dt}}">
...
<div class="row">
    ...
    <div class="col-12 col-md-3">
       ...
       min="{{$DSWorkmaster->AA_Dt}}" Max="{{$DSWorkmaster->Wo_Dt}}">
  ...
  <div class="row">
    ...
    <div class="col-12 col-md-3">
        <div class="form-group">
                ...
                class="form-control"  value="{{$DSWorkmaster->Agree_Dt }}"  Min="{{$DSWorkmaster->Wo_Dt }}" >
        </div>
    </div>
  </div>
 ...
 <div class="col-12 col-md-3">
    ...
    <div class="col-12 col-md-3">
        <div class="form-group">
          ...
          <input type="number" step="any" id="Period" name="Period" class="form-control wk_info1"
...
<select id="Perd_Unit" name="Perd_Unit" class="form-control" onchange="calculateCompletionDate()" style="max-width:250px;">
    <option {{ $DSWorkmaster->Perd_Unit == 'Day' ? 'selected' : '' }}>Day</option>
  <option {{ $DSWorkmaster->Perd_Unit == 'Month' ? 'selected' : '' }}>Month</option>
  <option {{ $DSWorkmaster->Perd_Unit == 'Year' ? 'selected' : '' }}>Year</option>
</select>
        </div>
      </div>
...
<div class="row">
    ...
    <div class="form-check">
      <input type="checkbox" id="" name="work_comp" class="form-check-input" value="{{$DSWorkmaster->work_comp ?? ''}}"  {{$DSWorkmaster->work_comp == 1 ? 'checked' : ''}}>
         ...
         <div class="modal-body">
            <form id="updateForm">
               <div class="form-group">
                  ...
                  <input type="hidden" class="form-control" id="tItemId"  name="tItemId">
               </div>
               <div class="form-group">
                  <label for="updateItemDescription">Sub NO</label>
                  <input type="text" class="form-control" id="updateSubNo"  name="">
               </div>
...
<!-- // when page load that time pagination laravel data  bind asyncronus laravel -->
<script>
                ...
                response.data.forEach(item => {
                    $('#tableBody').append(`
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-primary edit-button" data-row-id="${item.t_item_id}">Edit</button>
                            </td>
                            <td>
                                <button class="btn btn-danger delete-button" data-row-id="${item.t_item_id}">Delete</button>
                            </td>
                        </tr>
                    `);
                });
...
// when page load that time pagination laravel data  bind asyncronus laravel
    ...
    // <!-- //update tnditem -->
//when edit button click open model with particular that id //
   $(document).on('click', '.edit-button', function() {
    // Get the row ID from the button data attribute
    var tItemId = $(this).data('row-id');
    console.log(tItemId);
    $("#tItemId").val(tItemId);
    // Fetch item details using AJAX
    $.ajax({
      url: '{{ route("RouteEditTndItemsIndiviual", ":tItemId") }}'.replace(':tItemId', tItemId),
              method: 'GET',     
        
        success: function(response) {
                // Populate modal fields with the fetched data
            $("#updateItemNo").val(response.t_item_no);
            $("#updateSubNo").val(response.sub_no);
            $("#updateItemDescription").val(response.item_desc);
            $("#updateTenderedQty").val(response.tnd_qty);
            $("#updateItemUnit").val(response.item_unit);
            $("#updateTenderedRate").val(response.tnd_rt);
            $("#updateItemAmount").val(response.t_item_amt);
                // Show the modal
...
//when edit button click open model with particular that id //
   ...
   $("#updateButton").click(function() {
        // Check if tItemId is defined
        var tItemIdToUpdate = $("#tItemId").val();
        console.log(tItemIdToUpdate);
        if (typeof tItemIdToUpdate !== 'undefined') {
            // Update the selected row's values
            var updatedData = {
              tItemIdToUpdate: tItemIdToUpdate, // Pass tItemId
                itemNo: $("#updateItemNo").val(),
                subNo: $("#updateSubNo").val(),
...
console.log(updatedData);
            ...
            console.error("tItemId is not defined.");
        }
    });
});
...
function updateitem(updatedData) 
{
  $.ajax({
    ...
    success: function(response) {
      ...
      console.log("tItemIdToUpdate:", updatedData.tItemIdToUpdate);
  // Find the row in the table and update its content
var rowToUpdate = $('#tableBody').find(`[data-row-id="${updatedData.tItemIdToUpdate}"]`).closest('tr');
rowToUpdate.find(".text-right").eq(0).text(updatedData.itemNo);
rowToUpdate.find(".text-left").eq(0).text(updatedData.subNo); // Include sub_no here
rowToUpdate.find(".text-left").eq(1).text(updatedData.itemDescription);
rowToUpdate.find(".text-right").eq(1).text(updatedData.tenderedQty);
rowToUpdate.find(".text-left").eq(2).text(updatedData.itemUnit);
rowToUpdate.find(".text-right").eq(2).text(updatedData.tenderedRate);
rowToUpdate.find(".text-right").eq(3).text(updatedData.itemAmount);
 ...
 $(document).on('click', '.delete-button', function() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            deleteItemFromDatabase(tItemId, clickedButton); // Pass the clicked button reference
        }
    });
});
